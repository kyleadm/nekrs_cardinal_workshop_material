C-----------------------------------------------------------------------
      subroutine aux_outputRS(wd,ifexport)
      implicit none
      include 'SIZE'
      include 'TOTAL'
C
...
c     integer estrd,ipt,wpt
      ...
      real ypf(lx1,ly1,lz1,lelv),cff(lx1,ly1,lz1,lelv),dist
      real tau(3),norm(3),vsca,tauw
      real utau,rho,mu,yp
      real ypmin,ypmax,ypave,vol
      real glmin,glmax,glsum
      real vysv(lx1*ly1*lz1*lelt),vxsv(lx1*ly1*lz1*lelt)
      logical ifgrad
     ...
     &                           (gradu(i,j,k,i2,j2)+gradu(i,j,k,j2,i2))
                ...
                yp=dist*utau*rho/mu
                ypf(i,j,k,e) = yp
                cff(i,j,k,e) = tauw/(0.5*1.0*1.0*1.0)
                ypmin=min(ypmin,yp)
                ypmax=max(ypmax,yp)
      ...
      if(ifexport) then
        ...
        call copy(vxsv,vx,ntot)
        call copy(vx,ypf,ntot)

        call copy(vysv,vy,ntot)
        call copy(vy,cff,ntot)

        ifxyo=.true.
        ifvo =.true.

        call prepost(.true.,'aux')

        call copy(vx,vxsv,ntot)
        call copy(vy,vysv,ntot)
